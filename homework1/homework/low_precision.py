...
class Linear4Bit(torch.nn.Module):
    ...
    def _load_state_dict_pre_hook(
        ...
        if f"{prefix}weight" in state_dict:
            ...
            # Ensure the size is divisible by group_size
            if weight_flat.size(0) % self._group_size != 0:
                # Pad with zeros if necessary
                padding_size = self._group_size - (weight_flat.size(0) % self._group_size)
                weight_flat = torch.cat([weight_flat, torch.zeros(padding_size, dtype=weight_flat.dtype, device=weight_flat.device)])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            weight = weight_dequant[:self._shape[0] * self._shape[1]].view(self._shape)
            
            # Use functional linear
...
class BigNet4Bit(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # TODO: Implement me (feel free to copy and reuse code from bignet.py)
        #raise NotImplementedError()
         # Similar structure to BigNet but with 4-bit blocks
        self.model = torch.nn.Sequential(
