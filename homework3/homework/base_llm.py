...
class BaseLLM:
    ...
    def batched_generate(
        ...
        actual_num_return = num_return_sequences if num_return_sequences is not None else 1
        ...
        # Decode only the generated tokens (skip the input prompts)
        ...
        # Reshape if num_return_sequences was specified
        if num_return_sequences is not None:
            # Reshape flat list into list of lists
            answers = [
                answers[i:i + num_return_sequences] 
                for i in range(0, len(answers), num_return_sequences)
            ]
